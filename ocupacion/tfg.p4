...
control MyEgress(inout headers hdr,
    ...
    action add_swtrace(switchID_t swid) {
        hdr.mri.count = hdr.mri.count + 1;  //incrementa el contador de entradas
        hdr.swtraces.push_front(1); //inserta espacio para 1 nuevo elemento al inicio de la pila swtraces
        hdr.swtraces[0].setValid();  //en P4 hay que marcar las posiciones empujadas manualmenete
        hdr.swtraces[0].swid = swid; //escribe el ID del switch actual
        hdr.swtraces[0].qdepth = (qdepth_t)standard_metadata.deq_qdepth; //guardamos la ocupacion de la cola
	//ajustamos la longitud del paquete
